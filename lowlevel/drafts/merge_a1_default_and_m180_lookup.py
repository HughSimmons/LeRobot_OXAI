#!/usr/bin/env python3
"""Reproducibly merge a1 default-home and m180-home lookup successes."""
...
DEFAULT_BASELINE = LOWLEVEL_DIR / "a1_non_h_reverse_move_lookup.json.bak_20260622_195654"
DEFAULT_SUPPLEMENT = LOWLEVEL_DIR / "a1_non_h_reverse_move_lookup.json"
DEFAULT_OUTPUT = LOWLEVEL_DIR / "a1_non_h_reverse_move_lookup.json"
DEFAULT_MAP_OUTPUT = LOWLEVEL_DIR / "a1_non_h_reverse_move_lookup_success_map.svg"
...
def target_file(key: str, move: dict) -> str:
    return move_destination(key, move)[0]
...
def merge_lookups(
    ...
    *,
    ...
    for key, move in sorted(baseline_successes.items()):
        if target_file(key, move) in {"a", "b", "c", "d", "e"}:
            merged_moves[key] = stamp_move(
                move,
                home_joints_deg=default_home,
                lift_override=None,
                home_policy="default_home_restored_from_backup",
                provenance=str(baseline_path),
            )
    ...
    for key, move in sorted(supplement_successes.items()):
        if target_file(key, move) in {"f", "g", "h"}:
            ...
            merged_moves[key] = stamp_move(
                ...
                provenance=str(supplement_path),
            )
    ...
    metadata.update(
        {
            ...
            "source": "lowlevel/drafts/merge_a1_default_and_m180_lookup.py",
            "from_square": SOURCE_SQUARE,
            "source_squares": [SOURCE_SQUARE],
            "merge_policy": (
                "default-home backup for a-e targets; "
                "alternate shoulder-pan -180 supplement for f/g/h targets"
            ),
            "merge_baseline_path": str(baseline_path),
            "merge_supplement_path": str(supplement_path),
